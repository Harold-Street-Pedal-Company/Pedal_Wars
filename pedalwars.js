...
(function () {
  ...
  function addCash(v) {
    state.cash = Math.max(0, Math.floor(state.cash + v));
  }
  function adjustDebt(v) {
    state.debt = Math.max(0, Math.floor(state.debt + v));
  }
  function bumpRep(d) {
    state.rep = clamp(state.rep + d, 0, 1);
  }

  // ===== Actions =====
  gi("borrowBtn").onclick = () => {
    adjustDebt(+500);
    addCash(+500);
    log("Borrowed $500 at current APR.", "warn");
    renderStats();
  };
  gi("repayBtn").onclick = () => {
    if (state?.debt <= 0) {
      log("No debt to repay.", "warn");
      return;
    }
    if (state.cash <= 0) {
      log("No cash to repay.", "bad");
      return;
    }
    const pay = Math.min(500, state.debt, state.cash);
    adjustDebt(-pay);
    addCash(-pay);
    log("Repaid " + fmt(pay) + ".", "good");
    renderStats();
  };

  function buy(id, qty) {
    const cost = state.prices[id] * qty;
    if (capacityUsed() + qty > state.cap) {
      log("Not enough space.", "bad");
      return;
    }
    if (cost > state.cash) {
    ...
    bumpRep(+0.002 * qty);
    log(
      "Bought " + qty + " × " + ITEMS.find((x) => x.id === id).name + " for " + fmt(cost) + ".",
      "good"
    );
    renderStats();
    renderMarket();
  ...
  function sell(id, qty) {
    ...
    if (qty > have) {
      log("You do not own that many.", "bad");
      return;
    }
  ...
  gi("sellAllBtn").onclick = () => {
    ...
    addCash(net);
    log(
      "Quick sold everything for " + fmt(total) + (fee ? " (−" + fmt(fee) + " fee)" : "") + " → " + fmt(net) + " net.",
      "good"
    );
    renderAll();
  };
  gi("dumpBtn").onclick = () => {
    ...
    if (!owned.length) {
      log("You own nothing to dump.", "warn");
      return;
    }
    const it = pick(owned);
    state.inv[it.id] -= 1;
  ...
  gi("travelBtn").onclick = () => {
    travel(gi("locationSelect").value);
  };
  ...
  function travel(dest) {
    if (dest === state.location) {
      log("You are already there.", "warn");
      return;
    }
    const cost = travelCostFor(dest);
    if (state.cash < cost) {
      log("Travel costs " + fmt(cost) + ". You need more cash.", "bad");
      return;
    }
    addCash(-cost);
    ...
    state.location = dest;
    log(
      "Traveled " + from + " → " + to + " (" + (cost ? "cost " + fmt(cost) : "free") + ")",
      cost ? "warn" : "good"
    );
    // travel events only (no price change)
  ...
  function travelEvent() {
    ...
    if (r < 0.2) {
      const gain = Math.floor(50 + r * 250);
      addCash(gain);
      log("Scored a pop-up flip on arrival: +" + fmt(gain) + ".", "good");
    } else if (r < 0.4) {
      const loss = Math.min(state.cash, Math.floor(30 + r * 200));
      addCash(-loss);
      log("Road fees hit: −" + fmt(loss) + ".", "bad");
    } else if (r < 0.6) {
      bumpRep(+0.02);
      log("Met a demo artist — reputation up.", "good");
    } else if (r < 0.75) {
      const interest = Math.floor(state.debt * 0.001 * (1 + Math.floor(r * 3)));
      adjustDebt(+interest);
      log("Travel delays increased costs: +" + fmt(interest) + " debt.", "warn");
    } else if (r < 0.9) {
      const refund = Math.floor(20 + r * 120);
      addCash(refund);
      log("Returned a defective part and got " + fmt(refund) + " back.", "good");
    } else {
      bumpRep(-0.015);
      log("Buyer flaked on meetup — tiny rep hit.", "warn");
    }
  }
  ...
  function endDay() {
    ...
    if (fee > 0) {
      addCash(-fee);
      log("Storage fees " + fmt(fee) + ".", "warn");
    }
    dailyEvent();
    if (state.day < DAYS_LIMIT) {
      state.day++;
      genPrices(); // <-- only here
      renderAll();
    } else {
  ...
  function dailyEvent() {
    ...
    if (roll < 0.2) {
      bumpRep(+0.02);
      footer("Hype building…");
    } else if (roll < 0.35) {
      bumpRep(-0.01);
      footer("Market feels soft.");
    } else if (roll < 0.45) {
      const loss = Math.min(state.cash, Math.floor(50 + rng() * 200));
      addCash(-loss);
      bumpRep(-0.015);
      footer("Account took a ding.");
    } else if (roll < 0.55) {
      const owned = ITEMS.filter((i) => state.inv[i.id] > 0);
      if (owned.length) {
        const it = pick(owned);
        const take = Math.max(1, Math.floor(state.inv[it.id] * (0.25 + rng() * 0.5)));
        state.inv[it.id] = Math.max(0, state.inv[it.id] - take);
        footer("Paperwork error.");
      }
    } else if (roll < 0.7) {
      footer("Buzz is in the air.");
    } else {
      footer("Quiet day.");
    }
  }
  function footer(text) {
    gi("eventFooter").textContent = text;
  }

  // ===== Save/Load/Reset & Scores =====
  const SAVE_KEY = "pedalwars_save_v1",
    SCORE_KEY = "pedalwars_scores_v1";
  gi("saveBtn").onclick = () => {
    try {
  ...
  gi("resetBtn").onclick = () => {
    if (confirm("Reset game?")) {
      ...
      gi("startOverlay").style.display = "";
      gi("gameControls").style.display = "none";
      const ul = gi("log");
      if (ul) ul.innerHTML = "";
    }
  };
  ...
  function addScore(name, score, mode) {
    ...
    arr.push({ name, score, mode, date: new Date().toLocaleDateString() });
    arr.sort((a, b) => b.score - a.score);
    saveScores(arr.slice(0, 10));
  }
  function gameOver() {
    let liquidation = 0;
    for (const k in state.inv) liquidation += state.inv[k] * state.prices[k];
    const net = state.cash + liquidation - state.debt;
    const grade =
      net > 10000 ? "Legend" : net > 5000 ? "Pro" : net > 2000 ? "Hobbyist" : "Weekend Warrior";
  ...
  try {
    console.assert(typeof log === "function", "log() defined");
    console.log("[Pedal Wars] bundle executed");
  } catch (e) {}
})();
