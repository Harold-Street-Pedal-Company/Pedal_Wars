...
(function () {
  ...
  function computePricesFor(day, locId){
    ...
    const mood    = 0.90 + pr()*0.30;        // city mood 0.90..1.20
    ...
    ITEMS.forEach((it, idx)=>{
      ...
      price *= (0.92 + pr()*0.28);                 // per-item local noise
  ...
  function renderMarket(){
    ...
    // (No per-button onclicks here; delegation handles clicks)
  }
  ...
  function travel(dest){
    ...
    state.location=dest;
    // show delta vs that city's previous day
  ...
  function load(){ try{
      ...
      refreshPricesForCurrentDayAndLocation({ compareToPrevDay:true });
      gi('startOverlay')?.style && (gi('startOverlay').style.display='none');
      gi('gameControls')?.style && (gi('gameControls').style.display='flex');
      renderAll('Loaded save.'); setTimeout(renderStats,0);
    }catch(e){ console.warn('Load failed',e); } }
  function resetGame(){ if(confirm('Reset game?')){ try{ localStorage.removeItem('pedalwars_save_v1'); }catch(e){} state=null; DAYS_LIMIT=30; dailyCache={}; gi('startOverlay')?.style && (gi('startOverlay').style.display=''); gi('gameControls')?.style && (gi('gameControls').style.display='none'); gi('log')?.innerHTML=''; renderTravelCosts(); log('Game reset. Choose a mode to start.'); setTimeout(renderStats,0); } }
  ...
  // ===== Global delegated clicks (fixes “can’t click” issues) =====
  function getQtyFor(act, id){
    const input = gi(`${act === 'buy' ? 'b_' : 's_'}${id}`);
  ...
  function handleClick(e){
    ...
    // Always stop anchors/themes from hijacking
    e.preventDefault();
    e.stopPropagation();
    ...
    // Market buy/sell (data-act)
    const act = btn.getAttribute('data-act');
    if(act === 'buy' || act === 'sell'){
      const id = btn.getAttribute('data-id');
      const qty = getQtyFor(act, id);
      if(qty > 0){ act === 'buy' ? buy(id, qty) : sell(id, qty); }
      return;
    }

    // Top controls
    switch(btn.id){
      case 'borrowBtn': doBorrow(); return;
      case 'repayBtn' : doRepay(); return;
      case 'sellAllBtn': quickSellAll(); return;
      case 'dumpBtn'   : dumpOne(); return;
      case 'travelBtn' : travel(gi('locationSelect')?.value || state.location); return;
      case 'nextBtn'   : endDay(); return;
      case 'saveBtn'   : save(); return;
      case 'loadBtn'   : load(); return;
      case 'resetBtn'  : resetGame(); return;
    }
  }

  // Bind once (survives re-render, iframe wrappers, etc.)
  root.addEventListener('click', handleClick, { capture:false, passive:false });

  // Also wire start buttons on DOM ready for good measure
  function bindStartButtons(){
    const qb=gi('quickBtn'), nb=gi('normalBtn');
    qb && qb.addEventListener('click', handleClick);
    nb && nb.addEventListener('click', handleClick);
  }
  ...
  // Initial travel costs (visible before start)
  renderTravelCosts();
