<!-- ========== JS bundle to host externally as pedalwars.js ========== -->
<!-- Save the following as pedalwars.js on GitHub Pages/Netlify/etc. -->
<script type="text/plain" id="pedalwars-js-bundle">
(function(){
  // ===== Utilities =====
  ...
  // Build select immediately
  (function buildSelect(){ const sel=gi('locationSelect'); if(!sel) return; LOCATIONS.forEach(l=>{ const o=document.createElement('option'); o.value=l.id; o.textContent=l.name; sel.appendChild(o); }); })();

  // Bind start buttons (CSP-safe; no inline handlers)
  ...
  gi('resetBtn').onclick=()=>{ if(confirm('Reset game?')){ localStorage.removeItem(SAVE_KEY); gi('startOverlay').style.display=''; gi('gameControls').style.display='none'; gi('log').innerHTML=''; } };
